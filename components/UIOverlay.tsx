...
const UIOverlay: React.FC<UIOverlayProps> = ({ gameState }) => {
  ...
  return (
    <div className="absolute top-0 left-0 w-full h-full pointer-events-none p-8">
      ...
      {/* MIDI DEBUG - Toujours visible pour le setup */}
      <div className="absolute top-4 left-4 font-mono text-xs text-green-400 bg-black/80 border border-green-800 p-2 rounded z-[100] opacity-80">
          {gameState.lastMidiDebug}
      </div>

      {gameState.status === 'MENU' ? null : (
        <>
